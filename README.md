...
面向货币银行学课程的银行年报与货币政策报告问答工具。使用 pdfplumber 提取 PDF 文字、本地 BM25 检索证据，通过可配置的大模型生成带引用的回答，并提供 Streamlit 网页与命令行。
...
未配置模型时，项目仅展示检索原文和物理页码。需要生成回答时，在启动程序前设置环境变量：

```powershell
$env:RAG_BASE_URL = "https://你的服务地址/v1"
$env:RAG_MODEL = "你的模型名"
$env:RAG_API_KEY = "你的密钥"
```

请使用兼容 Chat Completions 的模型服务。本地模型可按服务要求省略密钥。`.env.example` 仅提供配置示例，程序不会自动读取 `.env`，不要将真实密钥提交到仓库。
...
每次问答只检索所选报告。对于未披露指标或证据不足的问题，应说明无法确定。扫描版 PDF 需要先完成 OCR；复杂表格中的数字建议对照原文复核。

详细说明见 [技术说明文档](技术说明文档.md)，新增报告的官方链接与验证记录见 [资料来源](资料来源.md)。
